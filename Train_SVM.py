...
x_train, x_test, y_train, y_test = train_test_split(
    images, labels, test_size=0.2, stratify=labels, random_state=100
)

x_train, y_train = augment_images(x_train, y_train)

X_train = np.array([extract_features_svm(img) for img in x_train])
X_test  = np.array([extract_features_svm(img) for img in x_test])
...
grid = GridSearchCV(svm, param_grid, cv=5, n_jobs=-1, verbose=2)
